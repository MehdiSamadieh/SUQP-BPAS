...
gg=gg*gg*2;
 gg==(gg+1);
  ...
  std::cerr << "ggold ==" << gg<< std::endl;

SparseUnivariateRationalPolynomial pol;
    pol.randomPolynomialU(  nterms,  coefBound, sparsity,  includeNeg);
    pol.setVariableName('y');
    std::cerr << "polold ==" << pol<< std::endl;
...
/* 
   std::cerr << "\nss ==" << ss<< std::endl;
  	Factors<SparseUnivariateRationalPolynomial> f=ss.squareFree();
   std::cerr<<f<<std::endl;   */
   //std::cerr<<f[2]<<std::endl;
 ...
 /////////////////////////////////////////////////////////////////////////////// GCD
   /* std::cerr << "\nss ==" << ss<< std::endl;
   std::cerr << "\ngg ==" << gg<< std::endl;
   std::cerr << "\ngcd ==" <<    ss.gcd(gg)<< std::endl;
 */
/////////////////////////////////////////////////////////////////// Psusedo divide
/*
 RationalNumber w=5;
 RationalNumber mult=6;
  std::cerr << "s ==" << ss<< std::endl;
std::cerr << "gg==" << gg<< std::endl;

ss.pseudoDivide( gg,&x,&mult) ;
std::cerr << "\nss ==" << ss<< std::endl;
std::cerr << "\ngg==" << gg<< std::endl;
std::cerr << "\nrem(X)==" << x<< std::endl;
...
/* std::cerr << "\n x: ==" << x<< std::endl;
 x=x.Integral(1); 
std::cerr << "Integral(x): ==" << x<< std::endl; */
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////devide 
//gg=ss/gg;
 //std::cerr << "\n  ss /gg: ==" << gg<< std::endl<< std::endl<< std::endl;
/////////////////////////////////////////////////////////////// devide by rational numer
 /*std::cerr << "ss: ==" << ss<< std::endl;
ss=ss/0;
 std::cerr << "\n  ss /2: ==" << ss<< std::endl;*/

 /////////////////////////////////////////////////////////////////
//ss=ss-RationalNumber(2);

 ////////////////////////////////////////////////////////// shift left //////////////////////////////////////////////////////////////////////////////////
  /*  ss=ss*2;
std::cerr << "ss: ==" << ss<< std::endl;
ss=ss>>2;
 std::cerr << "\n  ss new after >>2: ==" << ss<< std::endl;
 */
...
ss<<=2;
 std::cerr << "\n  s: ==" << ss<< std::endl; */
   ...
   /* std::cerr << "ss: ==" << ss.variable()<< std::endl;
o.setVariableName("e");
   std::cerr << "ss.e: ==" << ss<< std::endl; */


/* RationalNumber r(1,2);
    ss.setCoefficient(5,r);

       std::cerr << "\nss.new ==" << ss<< std::endl; */

   //std::cerr << "\nS.coeficient2: ==" << ss.coefficient(2)<< std::endl;
   //std::cerr << "S.evaluate: ==" << ss.evaluate(1) << std::endl;

 //std::cerr << "S: " << ss.evaluate(2) << std::endl;
 

  // ss.differentiate();
   //std::cerr<<"derevitave of s=="<< ss << std::endl;
  //std::cerr<<"derevitave of s=="<< ss.derivative(2) << std::endl;
  //std::cerr << "content of S: " << ss.content()<< std::endl;

  //std::cerr << "primitive part of S: " << ss.primitivePart()<< std::endl;

 // std::cerr << "numberof terms for S: " << ss << std::endl;
 // std::cout<<"degreee of ss==" <<ss.degree()<< std::endl;

  //std::cout<<"degreee of ss==" <<ss.numberOfTerms()<< std::endl;
 // std::cout<<"degreee of ss==" <<ss.leadingCoefficient()<< std::endl;

 // std::cout<<"degreee of ss==" <<ss.isConstantTermZero()<< std::endl;

  //std::cout<<"degreee of ss==" <<ss.trailingCoefficient<< std::endl;
...
//std:: cout <<"i=="<<i;s
 ...
 //std::cout<<"i=="<<i;
  ...
  AltArrU_t* aa=makePolynomial_AAU(10);
   AltArrU_t* bb=makePolynomial_AAU(10);
   AltArrU_t* cc=makePolynomial_AAU(10);
...
//aa = addPolynomials_AAU(aa,bb);


  sortPolynomial_AAU(aa);
  bb=deepCopyPolynomial_AAU(aa);

  
 AAElemU_t*c=multiplyTerms_AAU(&(aa->elems[1]),&(bb->elems[1]));
  ...
  addTerm_AAU(bb,d,k);
  negatePolynomial_AAU(bb);
/*   printAAU(aa);
  printAAU(bb) ; */
  
  //AltArrU_t* cc= addPolynomials_AAU( aa,  bb);

/////////////////////////////////////////////////////////////////////////////////////exponentiation
 /* std:: fprintf(stderr,"%s","bb====");
    printAAU(bb) ;

  bb=exponentiatePoly_AAU(bb,6)
  printAAU(bb) ;
//*p->poly=cc;
 */

 cc=exponentiatePoly_AAU(aa,2);
 cc=addPolynomials_AAU(aa,cc);
  ...
  /*  mpq_set_ui(k , 2, 1);
   ...
   std::fprintf(stderr, "polynomial cc := ");
   printAAU(cc) ;
    
     mpq_init(answer);

  //evaluatePoly_AAU(cc,  k, answer);
    //std::cerr << "S: " << k << std::endl;

gmp_fprintf(stderr, "\n%s%Qd","reference answer == ", answer); */
cc=exponentiatePoly_AAU(aa,2);
//std::fprintf(stderr,"aa=");
//printAAU(aa) ;
...
//printAAU(cc) ;
 bb=deepCopyPolynomial_AAU(aa);
   ...
   //mpq_set(k,a);
 //addTerm_AAU(bb,1 ,k);
   //sortPolynomial_AAU(bb);

 /* bb=addPolynomials_AAU(aa,aa);

std::fprintf(stderr,"bb==");
printAAU(bb) ;

dividePolynomials_AAU(bb,aa,&aa,&cc);
std::fprintf(stderr,"aa or quo==");
printAAU(aa) ;
std::fprintf(stderr,"cc or reminder ==");
printAAU(cc); */
...
printAAU(bb);
bb=integral_AAU(bb,1);
std::fprintf(stderr,"integral of aa ==");
...
printAAU(bb);
 ...
 cc =univariateGCD_AAU( aa,  bb); 
std::fprintf(stderr,"gcd (aa,bb) ==");
...
AltArrU_t ** A_=squareFree_AAU(ss.poly,&nfact);
std::fprintf(stderr,"\nfactors of c ==");
...
//mpz_t * tu= ConvertfromAlttoAr( aa);

// DUQP
/* DenseUnivariateIntegerPolynomial alpha(3),beta(3),gamma(3);
alpha.setVariableName('x');
beta.setVariableName('x');
gamma.setVariableName('x');
mpz_class one(1), two(2), mone(-1);
alpha.setCoefficient(2,one);
alpha.setCoefficient(0,mone);
beta.setCoefficient(2,one);
beta.setCoefficient(1,two);
beta.setCoefficient(0,one);
cout << "a = " << alpha << endl;
cout << "b = " << beta << endl;
gamma = alpha.gcd(beta,1);
cout << "g = " << gamma << endl; */

//AltArrU_t* result =modularGCD (aa , aa); 
